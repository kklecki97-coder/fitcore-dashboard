...
| | Cold Email | Instagram DMs |
|--|-----------|---------------|
| Setup time | 2-4 weeks (domains, warmup) | Same day |
| Open rate | 40-60% | 80-90% |
| Response rate | 3-5% | 10-20% |
| Trust factor | Low (stranger in inbox) | Higher (they see your profile, your content, you're a real person) |
| Scalability | High (automated sending) | Lower (manual sending required) |
| Ban risk | Domain burn if sloppy | Account ban if automated |

**Key rule:** Automate the research, never automate the sending. Instagram detects and bans automation aggressively.

---

## Step 1: Set Up Your Instagram Profile (Day 1)
...
## Step 2: Define Your Target (ICP for Instagram)
...
- Gym-based trainers only (no "online" in their setup)
- Coaches with 100K+ followers (too big, different needs)
- Coaches who are also selling courses about "how to be a coach" (different buyer)
- Accounts that haven't posted in 30+ days (inactive)
- Coaches who clearly already have a custom app or branded platform
...
|-------|---------|---------------|
| Instagram handle | @jakefitcoaching | Search/scrape |
| Full name | Jake Thompson | Bio or posts |
| Follower count | 8,200 | Profile |
| Bio summary | "Online fitness coach, 1:1 coaching, link to apply" | Profile |
| Niche | Weight loss for busy dads | Bio + recent posts |
| Website/link in bio | jakefitcoaching.com | Profile |
| Location (if visible) | Austin, TX | Bio |
| Notes for personalization | "Just posted about hitting 30 clients" | Manual review |

### Automation Opportunity
A script can handle lead discovery:
- Search Instagram hashtags and pull coach profiles
- Filter by follower count (1K-50K)
...
## Step 4: Warm Engagement Before the DM (24-48 Hours Before)
...
- "That progressive overload breakdown is solid. Do you use the same structure for your online clients?"
- "30 clients and still doing weekly video check-ins? That's dedication"
- "The before/after on this client is insane. How long was the program?"

### Bad Comments (Don't Do This)
- "Great post!" / "Love this!" / "So true!"
- Anything with fire/muscle emojis only
- Anything about your product
- Anything generic that could apply to any post

### Timing
- Follow + like on **Day 0**
- Comment on **Day 0 or Day 1**
- DM on **Day 1 or Day 2** (24-48 hours after the first engagement)

This way when they see your DM, they think: "Oh, this is the guy who left that comment on my post." You're no longer a stranger.

---

## Step 5: Send the DM (Daily Execution)

### The Rules
1. **Keep it under 3 sentences.** Coaches get 50+ DMs a day. Long messages get skipped.
2. **Make it about them, not you.** Reference their content, their coaching, their clients.
3. **Ask a question, don't pitch.** Start a conversation. The pitch comes later.
4. **No links in the first message.** Instagram suppresses DMs with links.
5. **No copy-paste.** Every message must feel personal. If it could be sent to anyone, rewrite it.
...
## Step 6: Track Everything (Pipeline Management)

### Tracking Fields
| Field | Values | Purpose |
|-------|--------|---------|
| Handle | @username | Who |
| Name | First name | Personalization |
| Date followed | 2026-02-21 | Timing |
| Date engaged (like/comment) | 2026-02-22 | Timing |
| Date DMed | 2026-02-23 | Timing |
| DM status | sent / replied / no reply | Pipeline |
| Conversation status | cold / talking / interested / call booked / closed / dead | Pipeline |
| Follow-up date | 2026-02-26 | Don't forget |
...
2. Pick 10-15 coaches to engage with today (like + comment)
...
2. Add new leads for tomorrow's engagement
...
|----------|-------------|
| New leads added to list | 20-30 |
| Profiles engaged (follow + like + comment) | 10-15 |
| New DMs sent | 8-12 |
| Follow-ups sent | 3-5 |
| Replies handled | All of them |
...
| Reply rate | 15-25% |
| Replies | 38-88 |
| Conversations that lead to demo | 20-30% of replies |
| Demos/calls booked | 8-26 |
| Close rate on demos | 20-30% |
| **New clients per month** | **2-8** |

At $300-500 per client setup + $50-100/month recurring, that's $600-4,000 in setup fees + growing recurring revenue in the first month alone.
...
Instagram will ban your account if you use automation tools for DMs. This includes:
- IGLeads, Jarvee, or any DM automation tool
- Browser extensions that auto-send messages
- Any tool that logs into your account and performs actions
...
| Task | Automate? | Tool/Script |
|------|----------|-------------|
| Finding coach profiles by hashtag/keywords | Yes | Apify / PhantomBuster / custom script |
| Filtering by follower count, bio keywords | Yes | Python script |
| Building and maintaining the lead list | Yes | Script → Google Sheet / Supabase |
| Follow-up reminders | Yes | Script that checks tracker dates |
...
python3 jakub/execution/find_instagram_leads.py --dry-run

# Search hashtags only (cheapest, ~$0.76 for 100/hashtag)
python3 jakub/execution/find_instagram_leads.py --mode hashtag --limit 100

# Search hashtags + keywords (more leads, ~$2.56)
python3 jakub/execution/find_instagram_leads.py --mode both --limit 100

# Save to both CSV and Supabase
python3 jakub/execution/find_instagram_leads.py --mode hashtag --output both

# Adjust follower range (recommended: 2000-40000 for quality leads)
python3 jakub/execution/find_instagram_leads.py --min-followers 2000 --max-followers 40000

# Bigger batch for more leads (~$2.76)
python3 jakub/execution/find_instagram_leads.py --mode hashtag --limit 300 --output both --min-followers 2000 --max-followers 40000
```

Output: CSV in `jakub/.tmp/` and/or `instagram_leads` table in Supabase with outreach tracking fields (status, followed_at, engaged_at, dmed_at, follow_up_at, notes).

#### How to Add New Leads (Full Process for Future Agents)

**Step 1: Supabase table must already exist.** If starting fresh, create it manually via Supabase SQL Editor:
```sql
...
**Step 3: If the script's Supabase push fails** (it uses `Prefer: resolution=ignore-duplicates` which can silently skip all rows), push from CSV manually using upsert:
```python
# Push CSV leads with proper upsert (on_conflict handles duplicates correctly)
# The key header is: 'Prefer': 'return=minimal,resolution=ignore-duplicates'
...
```bash
# Quick count check via REST API HEAD request with Prefer: count=exact
# The Content-Range header returns the total: e.g., "0-511/512"
```

#### Known Issues & Learnings
- **Apify actor IDs use `~` not `/` in the REST API.** The script handles this with `.replace("/", "~")`. E.g., `apify/instagram-hashtag-scraper` becomes `apify~instagram-hashtag-scraper` in API calls.
- **Apify free tier = $5/month.** A typical hashtag run (10 hashtags × 100 results + profile scraping) costs ~$0.76. A bigger run (300 limit) costs ~$2.76. Both modes combined ~$5.36 (exceeds free tier).
- **Follower range 2000-40000 produces better quality** than the default 1000-50000. Cuts out small/inactive accounts.
- **The script always saves a CSV to `jakub/.tmp/`** regardless of output mode. Filenames include timestamps.
...
python3 jakub/execution/generate_dm_drafts.py

# Limit to N leads
python3 jakub/execution/generate_dm_drafts.py --limit 20

# Dry run (see which leads would get DMs)
python3 jakub/execution/generate_dm_drafts.py --dry-run

# Regenerate DMs (overwrite existing)
python3 jakub/execution/generate_dm_drafts.py --regenerate
```

Cost: ~$0.005/lead with gpt-4o-mini. Run this before your daily DM session so drafts are ready in the dashboard.

### Pipeline Dashboard (Daily Tasks View)
...
```sql
ALTER TABLE instagram_leads ADD COLUMN IF NOT EXISTS dm_draft TEXT;
```
...
Instagram DMs and cold email are complementary:
- **Email first, Instagram second:** If a coach opens your email but doesn't reply, find them on Instagram and engage there. The multi-channel touch increases conversion.
- **Instagram first, email second:** If a coach replies to your DM but goes cold, add them to a softer email sequence.
- **Cross-reference leads:** Don't DM someone on Instagram AND email them at the same time. Pick one channel per lead, or stagger them by a week.

See `cold_email_outreach.md` for email-specific details.

---

## Getting Started Checklist

- [ ] Set up Instagram profile (Step 1)
- [ ] Create 2-3 posts to pin (dashboard screenshots, screen recordings)
- [ ] Find first batch of 50 coaches manually (Step 2 + 3)
- [ ] Start engaging with 10-15 per day (Step 4)
- [ ] Send first DMs 24-48 hours after engaging (Step 5)
- [ ] Set up tracking sheet/table (Step 6)
- [ ] Refine and repeat daily
