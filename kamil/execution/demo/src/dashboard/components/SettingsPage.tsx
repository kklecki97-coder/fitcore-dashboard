...
import { Sun, Moon, User, Bell, Shield, Palette, X, Save, CheckCircle, CreditCard, Camera, Trash2, Copy, AlertTriangle, Plus, Edit3 } from 'lucide-react';
      ...
      <div style={{ ...styles.grid, gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)' }}>
        ...
        <GlassCard delay={0.25}>
          <div style={styles.sectionHeader}>
            ...
            <div>
              <h3 style={styles.sectionTitle}>Plans & Pricing</h3>
              <p style={styles.sectionSub}>Create your own coaching plans</p>
            </div>
          </div>
          <div style={styles.divider} />
          {[
            { name: 'Gold Plan', price: 122, billing: 'mo', type: 'Training plan' },
          ].map((plan, idx) => (
            <div key={plan.name} style={{ ...styles.integrationRow, borderBottom: idx === 0 ? 'none' : '1px solid var(--glass-border)' }}>
              <div style={{ flex: 1 }}>
                <span style={styles.settingLabel}>{plan.name}</span>
                <div style={styles.settingDesc}>
                  ${plan.price}/{plan.billing} · {plan.type}
                </div>
              </div>
              <div style={{ display: 'flex', gap: '4px' }}>
                <button style={{ ...styles.disconnectBtn, padding: '6px 8px' }}><Edit3 size={14} /></button>
                <button style={{ ...styles.disconnectBtn, padding: '6px 8px', color: 'var(--accent-danger)' }}><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
          <div style={{ padding: '12px 20px' }}>
            <button style={{ ...styles.connectBtn, borderColor: 'var(--accent-primary)', color: 'var(--accent-primary)', width: '100%', justifyContent: 'center' }}>
