...
const pages = [
  { name: '1-overview',   nav: 'Overview',   wait: 2000 },
  { name: '2-clients',    nav: 'Clients',    wait: 2000 },
  { name: '3-programs',   nav: 'Programs',   wait: 2000 },
  { name: '4-messages',   nav: 'Messages',   wait: 2000 },
  { name: '5-checkins',   nav: 'Check-Ins',  wait: 2000 },
  { name: '6-payments',   nav: 'Payments',   wait: 2000 },
  { name: '7-analytics',  nav: 'Analytics',  wait: 2000 },
];
...
async function run() {
  ...
  await page.evaluate(() => {
    ...
    const loginBtn = buttons.find(b => /sign in|log in|login/i.test(b.textContent));
  ...
  for (const p of pages) {
    ...
    await page.evaluate((navText) => {
      ...
      const navItem = items.find(el => {
        ...
        return text === navText && el.offsetParent !== null && el.children.length === 0;
      });
      if (navItem) {
        navItem.click();
